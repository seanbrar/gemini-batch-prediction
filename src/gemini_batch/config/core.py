# src/gemini_batch/config/core.py
...
class FieldOrigin:
    ...
    file: str | None = None  # e.g., "~/.config/gemini_batch.toml"
...
def _origin_label(field: str, where: FieldOrigin) -> str:
    match where.origin:
        ...
        case Origin.PROJECT:
            ...
            return f"file:{file}"
        case Origin.HOME:
            file = where.file or "~/.config/gemini_batch.toml"
            return f"file:{file}"
        case _:
...
def main() -> int:  # pragma: no cover - thin utility  # noqa: D103
    ...
    parser = argparse.ArgumentParser("gb-config")
